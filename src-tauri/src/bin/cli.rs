    ...
    match cli.command {
        ...
        Command::Status => {
            ...
            for provider in app_config.providers {
                ...
                println!("{} [{}] {} -> {} ({})", if active { "*" } else { "-" }, provider.provider, provider.model_name, provider.base_url, provider.masked_key());
            }
        }
        Command::Serve { host, port } => {
            let (_paths, mut app_config) = config::load_or_init_config()?;
            if let Some(host) = host { app_config.host = host; }
            if let Some(port) = port { app_config.port = port; }
        ...
        Command::Test => {
            let (_paths, app_config) = config::load_or_init_config()?;
