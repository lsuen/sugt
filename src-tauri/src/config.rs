...
pub fn ensure_paths() -> Result<AppPaths> {
    ...
    fs::create_dir_all(&paths.config_dir).with_context(|| format!("无法创建配置目录 {}", paths.config_dir.display()))?;
    Ok(paths)
}
...
pub fn load_or_init_config() -> Result<(AppPaths, AppConfig)> {
    ...
    let raw = fs::read_to_string(&paths.config_file).with_context(|| format!("无法读取配置文件 {}", paths.config_file.display()))?;
...
pub fn save_config(paths: &AppPaths, config: &AppConfig) -> Result<()> {
    ...
    fs::write(&paths.config_file, serialized).with_context(|| format!("无法写入配置文件 {}", paths.config_file.display()))
}

pub fn normalize_config(config: &mut AppConfig) {
    for provider in &mut config.providers {
        provider.base_url = crate::model::trim_base_url(provider.base_url.clone());
    }
...
fn default_config_from_env() -> AppConfig {
    ...
    if let (Some(base_url), Some(api_key), Some(model_name)) = (base_url, api_key, model_name) {
        let provider = ProviderConfig::new("默认模型", "OpenAI Compatible", base_url, api_key, model_name);
...
mod tests {
    ...
    fn normalizes_base_url() {
        let mut config = AppConfig::default();
        config.providers.push(ProviderConfig::new("n", "p", "https://example.com/v1///", "k", "m"));
        normalize_config(&mut config);
